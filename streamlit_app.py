...
def _read_uploaded_preview(file_name: str, payload: bytes) -> pd.DataFrame:
    return read_uploaded_file(io.BytesIO(payload), file_name)
...
def _scope_attr(selection: object | None, name: str, default: str = "All") -> str:
    value = getattr(selection, name, default) if selection is not None else default
    if value is None or str(value).strip() == "":
        return default
    return str(value)


def _scope_date_text(selection: object | None, df: pd.DataFrame | None) -> str:
    label = _scope_attr(selection, "date_range_label", "All")
    date_range = getattr(selection, "date_range", None) if selection is not None else None
    ...
    if isinstance(df, pd.DataFrame) and not df.empty and "trade_date" in df.columns:
        ...
        if not dates.empty:
            return f"{label}: {dates.min():%Y-%m-%d} to {dates.max():%Y-%m-%d}"
    return label


def _scope_nunique(df: pd.DataFrame | None, column: str) -> int:
...
def render_advanced_audit_gateway(
    ...
    workbench_selection: object | None = None,
    filtered_market_df: pd.DataFrame | None = None,
    filtered_issuer_df: pd.DataFrame | None = None,
):
...
# Snapshot methodology/takeaway helpers live in ui/snapshot.py.


# Focused upload audit renderer lives in ui/upload.py.
...
# Validation report UI lives in ui/upload.py.
...
if bond_payload is not None:
    raw_bonds_preview = _read_uploaded_preview(bond_payload[0], bond_payload[1])
...
for trade_name, trade_bytes in trade_payloads:
    try:
        raw_trade_preview = _read_uploaded_preview(trade_name, trade_bytes)
...
if mmd_payload is not None:
    try:
        ...
        raw_mmd_preview = _read_uploaded_preview(mmd_name, mmd_bytes)
...
workbench_filtered_issuer = workbench_state.get("filtered_issuer_df") if isinstance(workbench_state, dict) else None
...
advanced_scope_state = render_advanced_audit_gateway(
    ...
    benchmark_source_mode=benchmark_source_mode,
    workbench_selection=workbench_selection,
...
advanced_group = st.segmented_control(
    "Advanced analysis group",
    advanced_group_options,
    default="Core Evidence",
    key="advanced_analysis_group",
    label_visibility="collapsed",
)
if not advanced_group:
    advanced_group = "Core Evidence"

if advanced_group == "Core Evidence":
    # -----------------------------------------------------------------------------
...
elif advanced_group == "Benchmark & RV":
    section_anchor("opportunity-workbench", "Opportunity Workbench")
    st.markdown(
        """
<div class="focus-band">
  <b>One workflow:</b> rank CUSIPs, inspect RV positioning, read the narrative reason, then save names to the watchlist.
  <div class="sidebar-nav-small" style="margin-top: 6px;">
    <a href="#security-screener">Ranking</a> ·
    <a href="#rv-positioning">RV Positioning</a> ·
    <a href="#market-narrative">Narrative</a> ·
    <a href="#recommendation-engine">Recommendation</a> ·
    <a href="#watchlist">Watchlist</a>
  </div>
</div>
<a id="security-screener"></a>
""",
        unsafe_allow_html=True,
    )
...
elif advanced_group == "Curve & History":
...
elif advanced_group == "Flow & Scenario":
    section_anchor("market-narrative", "Opportunity Workbench — Market Narrative")
...
This is a **proxy**, not true dealer inventory. True dealer behavior would require richer MSRB/dealer-side fields, bid-wanted data, or inventory data.
    ...
    section_anchor("rv-positioning", "Opportunity Workbench — Relative Value Positioning")
...
elif advanced_group == "Narrative & Review":
...
- This is not a database-backed permanent watchlist yet; that would be a future production feature.
    ...
    section_anchor("recommendation-engine", "Opportunity Workbench — Recommendation Narrative")
...
This is a screening commentary, not an investment recommendation. It should be reviewed alongside credit fundamentals, call structure, tax status, and actual executable market levels.
    ...
    with st.expander("Experimental / Later: AI Commentary Studio", expanded=False):
        st.caption("Optional AI writing layer. Keep collapsed during normal analyst review.")
        show_ai_commentary_studio = st.checkbox(
            "Load AI Commentary Studio",
            value=False,
            key="show_ai_commentary_studio",
            help="Loads the optional OpenAI-assisted commentary workflow only when needed.",
        )

    if show_ai_commentary_studio:
...
with st.expander("Executive Snapshot / Legacy Summary", expanded=False):
...
with st.expander("Raw / Audit Tables", expanded=False):
    st.subheader("Security Reference / Optional Bond Enrichment")
    ...
    st.subheader("Underlying Trade Detail")
...
with st.expander("Admin / Methodology Reference", expanded=False):
    st.markdown(
        """
...
"""
    ...
    st.subheader("Rating Spread Assumptions")
    safe_dataframe(rating_spread_table(), width="stretch", hide_index=True)

    st.subheader("Duration Proxy Assumptions")
    ...
    st.subheader("Version / Change Log")
    version_rows = [
...
if show_raw_tables:
    with st.expander("Raw / Processed Tables", expanded=False):
